...
CREATE USER IF NOT EXISTS 'reporting'@'%' IDENTIFIED BY '${MYSQL_REPORTING_PASSWORD:-reporting_pass}';

-- Maintenance user: needs broader access for OPTIMIZE/ANALYZE
CREATE USER IF NOT EXISTS 'maintenance'@'%' IDENTIFIED BY '${MYSQL_MAINTENANCE_PASSWORD:-maintenance_pass}';
...
CREATE TABLE IF NOT EXISTS `dim_date` (
  ...
  `created_at`        DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP,
  UNIQUE INDEX `uq_date_key` (`date_key`),
  UNIQUE INDEX `uq_full_date` (`full_date`),
...
CREATE TABLE IF NOT EXISTS `fact_stock_price` (
  ...
  INDEX `ix_stock_trade_date` (`stock_id`, `trade_date`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `fact_stock_news` (
  ...
  `llm_risks`         TEXT           NULL,
  `sentiment_score`   DOUBLE         NULL COMMENT '-1.0 to 1.0',
  `sentiment_label`   ENUM('very_negative','negative','neutral','positive','very_positive') NULL,
  `relevance_score`   DOUBLE         NULL COMMENT '0 to 1',
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- FINANCE — Internal Layer (computed signals, models, metadata)
-- ═══════════════════════════════════════════════════════════════════════════════
USE `finance_internal`;

-- ─── fact_forecast ───────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS `fact_forecast` (
  `id`                  BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  `stock_id`            BIGINT UNSIGNED NOT NULL COMMENT 'FK to finance_silver.dim_stock',
  `ticker`              VARCHAR(20)    NOT NULL,
  ...
  `predicted_direction` TINYINT        NULL COMMENT '1=up, -1=down, 0=flat',
  `confidence_score`    DOUBLE         NULL COMMENT '0 to 1',
...
CREATE TABLE IF NOT EXISTS `fact_model_registry` (
  ...
  `model_type`        VARCHAR(50)    NOT NULL COMMENT 'lightgbm, xgboost, ensemble',
  `ticker`            VARCHAR(20)    NULL COMMENT 'NULL = global model',
  `hyperparameters`   JSON           NULL,
  `training_rows`     INT UNSIGNED   NULL,
  `training_start`    DATE           NULL,
  `training_end`      DATE           NULL,
  `validation_metric` VARCHAR(50)    NULL,
  `validation_score`  DOUBLE         NULL,
  `artifact_path`     VARCHAR(500)   NULL COMMENT 'Path to serialized model file',
  `is_active`         TINYINT(1)     NOT NULL DEFAULT 1,
  `promoted_at`       DATETIME       NULL,
  `retired_at`        DATETIME       NULL,
  `created_at`        DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP,
  INDEX `ix_model_name` (`model_name`, `model_version`),
  INDEX `ix_model_active` (`is_active`, `model_name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- ─── fact_feature_importance ─────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS `fact_feature_importance` (
  `id`                BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  `model_registry_id` BIGINT UNSIGNED NOT NULL COMMENT 'FK to fact_model_registry',
  `feature_name`      VARCHAR(100)   NOT NULL,
  `importance_score`  DOUBLE         NOT NULL,
  `importance_rank`   INT UNSIGNED   NOT NULL,
  `importance_type`   VARCHAR(50)    DEFAULT 'gain' COMMENT 'gain, split, shap',
  `created_at`        DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP,
  INDEX `ix_feat_model` (`model_registry_id`),
  INDEX `ix_feat_rank` (`importance_rank`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;


-- ═══════════════════════════════════════════════════════════════════════════════
-- FINANCE — Gold Layer (final views/aggregates for consumption)
-- ═══════════════════════════════════════════════════════════════════════════════
USE `finance_gold`;

-- ─── fact_portfolio_snapshot ─────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS `fact_portfolio_snapshot` (
  `id`                  BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  `snapshot_date`       DATE           NOT NULL,
  `ticker`              VARCHAR(20)    NOT NULL,
  `stock_id`            BIGINT UNSIGNED NOT NULL,
  `signal_direction`    TINYINT        NULL COMMENT '1=long, -1=short, 0=neutral',
  `signal_strength`     DOUBLE         NULL COMMENT '0 to 1',
  `predicted_return`    DOUBLE         NULL,
  `confidence`          DOUBLE         NULL,
  `risk_score`          DOUBLE         NULL COMMENT '0=low risk, 1=high risk',
  `sector`              VARCHAR(100)   NULL,
  `close_price`         DECIMAL(18,6)  NULL,
  `market_cap`          DECIMAL(22,2)  NULL,
  `created_at`          DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP,
  UNIQUE INDEX `uq_portfolio_date_ticker` (`snapshot_date`, `ticker`),
  INDEX `ix_portfolio_date` (`snapshot_date`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- ─── fact_signal_performance ─────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS `fact_signal_performance` (
  `id`                  BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  `evaluation_date`     DATE           NOT NULL,
  `ticker`              VARCHAR(20)    NOT NULL,
  `signal_date`         DATE           NOT NULL,
  `signal_direction`    TINYINT        NOT NULL,
  `predicted_return`    DOUBLE         NULL,
  `actual_return_1d`    DOUBLE         NULL,
  `actual_return_5d`    DOUBLE         NULL,
  `actual_return_20d`   DOUBLE         NULL,
  `was_correct_1d`      TINYINT(1)     NULL,
  `was_correct_5d`      TINYINT(1)     NULL,
  `pnl_1d`              DOUBLE         NULL,
  `pnl_5d`              DOUBLE         NULL,
  `created_at`          DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP,
  INDEX `ix_sigperf_eval` (`evaluation_date`),
  INDEX `ix_sigperf_ticker` (`ticker`, `signal_date`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- ─── vw_daily_signals (view) ─────────────────────────────────────────────────
-- This view joins the latest forecast with silver-layer price + technicals
-- for a single-query dashboard feed. References cross-database tables.
CREATE OR REPLACE VIEW `vw_daily_signals` AS
SELECT
  f.`forecast_date`,
  f.`ticker`,
  f.`predicted_return`,
  ...
  f.`llm_rationale`,
  f.`model_name`
FROM `finance_internal`.`fact_forecast` f
WHERE f.`forecast_date` = (
  SELECT MAX(`forecast_date`) FROM `finance_internal`.`fact_forecast`
)
