...
Default to open-source libraries and tools wherever possible. Use free, publicly available data sources (yfinance, FRED, NewsAPI) over paid APIs. Prefer open-source ML libraries (LightGBM, XGBoost, scikit-learn) for tabular data. Exceptions: LLMs may use Anthropic Claude or Google Gemini when explicitly configured via environment variables.
